        ...
        <el-card class="active-card">
          ...
          <div v-if="activeRecord" class="active-details">
            <div class="detail-grid">
              <div>
                <label>Order</label>
                <div>{{ activeRecord.order_number }} · {{ activeRecord.product_name }}</div>
              </div>
              <div>
                <label>Operation</label>
                <div>{{ activeRecord.operation_name || activeRecord.operation_number }}</div>
              </div>
              <div>
                <label>Started</label>
            ...
            <div class="active-actions">
              ...
              <el-button type="danger" @click="openClockOutDialog(activeRecord)">Clock Out</el-button>
            </div>
          </div>
          <el-empty
            v-else
        ...
        <el-table-column label="Operation" width="220">
          <template #default="scope">
            <div class="operation-cell">
              ...
              <span class="secondary-text">Op {{ scope.row.operationNumber }}</span>
            </div>
          </template>
        </el-table-column>
...
import { getWorkstations, getStaff } from '@/modules/basic-data/services/basicDataService';
...
const workstationOptions = computed(() =>
  workstations.value
    ...
    .map(ws => ({
      id: ws.id,
      label: ws.production_line_name ? `${ws.name} (${ws.production_line_name})` : ws.name,
...
const selectedWorkstationName = computed(() => {
  const current = workstations.value.find(ws => ws.id === selectedWorkstation.value);
...
const pendingTasks = computed(() => scheduledTasks.value.filter(task => {
...
const buildCompletedIndex = (records) => {
  ...
  records.forEach(record => {
    if (record.component) {
      ...
      const existingTime = new Date(existing.end_time || existing.timestamp || 0).getTime();
      const incomingTime = new Date(record.end_time || record.timestamp || 0).getTime();
...
const refreshWorkstationData = async () => {
  ...
  try {
    const [scheduleData, activeData, completedData] = await Promise.all([
      ...
      getProductionCounting({ workstation: selectedWorkstation.value, status: 'in_progress' }),
      getProductionCounting({ workstation: selectedWorkstation.value, status: 'completed' }),
    ]);
    ...
    completedRecordIndex.value = buildCompletedIndex(normalizeListResponse(completedData));
  } catch (error) {
...
const clockIn = async (task) => {
  ...
  if (task.productionLineId && selectedWorkstationLine.value && task.productionLineId !== selectedWorkstationLine.value) {
    ElMessage.warning('Selected workstation is not part of the order production line');
    return;
  }
  try {
...
watch(selectedProductionLine, () => {
  ...
  const first = workstationOptions.value.find(ws => ws.productionLine === selectedProductionLine.value);
